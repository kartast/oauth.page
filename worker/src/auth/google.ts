...
google.get("/callback", async (c) => {
  ...
  try {
    ...
    if (flowType === "visitor") {
      ...
      return new Response(
        `<!DOCTYPE html><html><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>Logged in</title><style>body{font-family:system-ui,-apple-system,sans-serif;background:#0a0a0a;color:#fff;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0}.card{max-width:420px;text-align:center;padding:24px;background:#111;border:1px solid #262626;border-radius:12px}.ok{font-size:28px;margin-bottom:8px}.muted{color:#a1a1aa}</style></head><body><div class="card"><div class="ok">✓</div><h2>You’re logged in</h2><p class="muted">Redirecting you back to the site…</p><p><a href="${redirectTo}" style="color:#a78bfa">Continue now</a></p></div><script>setTimeout(()=>location.href=${JSON.stringify(redirectTo)},900)</script></body></html>`,
        {
          status: 200,
          headers: {
